...
The library is **vendored**, at `identicon/reference/vendor/`, so the vectors
can be re-derived offline by anyone for as long as this repository exists — and
`tests/test_conformance.py` re-derives them on every run. A reference that has
to be downloaded is a reference that can disappear, and one did during the week
this was written. It is a test fixture: nothing in the evaluator, the plasmoid
or the CLI imports it, and it ships in nothing. `NOTICE` records the licence and
...
AGPL-3.0-or-later. See `LICENSE`. One third-party library is vendored as a test
fixture — `stewartlord/identicon.js`, BSD-2-Clause, at
`identicon/reference/vendor/`, with its licence retained beside it; `NOTICE`
